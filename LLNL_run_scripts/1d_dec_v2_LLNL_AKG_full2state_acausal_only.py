...
def main(path_base, rat_name, day, epoch, shift_amt, path_out):
    ...
    use_enc_as_dec = 0
    decode_all = 1   # dandecode all times, run classifier according to use_enc_as_dec

    discrete_tm_val=.98   # for classifier; consistent with 100ms average staying in a state
    ...
    posterior_file_name = os.path.join(path_out,  rat_name + '_' + str(day) + '_' + str(epoch) + '_shuffle_' + str(shift_amount) + '_posteriors_v2.nc')
    ...
    position_file_name = os.path.join(path_out, rat_name + '_' + str(day) + '_' + str(epoch) + '_shuffle_' + str(shift_amount) + '_linearposition_v2.nc')
    ...
    # generate mask for classifier, since with  decode_all there will be no nans to indicate how to chunk out when to run it
    if decode_all:
        ...
        velmask = None # no velmask = classifier will be run on full epoch
    else:
        velmask = None
    ...
    # run original 3state classifier 
    ...
    # for just continuous vs fragemnted, 2state version
