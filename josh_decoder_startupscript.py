...
def main(argv):
    ...
    # ensure that all processes that needed to set up their network clients, did so successfully
    comm.Barrier()
    ...
    if config['datasource'] == 'trodes':
        if rank == config['rank']['supervisor']:
            ...
            print("Main process Trodes network deleted")
        else:
            network.closeConnections()
            del network
            print('Non main process Trodes network deleted')
    exit(0)
